...
public class ArvoreB 
{
    ...
    public void remaneja(No folha, int pos)
    {
        ...
        {
            folha.setPosArq(i, folha.getPosArq(i-1));
    ...
    public void split(No folha, No pai, int info) //esse info pode ser declarado dentro da função pq ele nao precisa retornar
    {
        ...
        {
            cx2.setPosArq(i-(No.getN()+1), folha.getPosArq(i));
        ...
        if(pai == folha)
        {
            folha.setPosArq(0, folha.getPosArq(No.getN())); 
        ...
        else
        {
            ...
            pai.setPosArq(pos, folha.getPosArq(No.getN()));
    ...
    public void inOrdemPosArq(No raiz)
    {
        int i;
        if(raiz != null)
        {
            for (i=0; i<raiz.getTl(); i++)
            {
                inOrdemPosArq(raiz.getLig(i));
                System.out.print(raiz.getPosArq(i) + " ");
            }
            inOrdemPosArq(raiz.getLig(i));
        }
    }
...
///////////////////////////////////exclusao/////////////////////////////////////////////////    
    public void exclui(int info)
    {
        ...
        if (p != null)
        {
            ...
            if(p.getLig(0) != null) //se não for folha - substitui o valor a ser retirado pelo valor adjacente na folha
            {
               ...
               int posArq = noAux.getPosArq(noAux.getTl()-1);
               p.setInfo(pos, info);
               p.setPosArq(pos, posArq);
               p = noAux;
            ...
            {
                //reestruturando 
                No pai = localizaPai(p, info);  //parece q ta errado pq info ja foi excluido , ou da pra achar o pai mesmo assim?
    ...
    public int busca(int info)   //retorna o posArq do registro buscado
    {
        No aux = encontraNo(info);
        if (aux!=null)
            return aux.getPosArq(contem(aux, info));
        else
        {
    ...
    public void remanejaParaExcluir(No caixa,int pos)   //eh diferente do remaneja pq eh pra excluir
    {
        ...
        {
            caixa.setPosArq(i, caixa.getPosArq(i+1));
    ...
    public void corrige(No p, No pai)   //desfaz split
    {
        ...
        { 
            ...
            if(posLig>0 && pai.getLig(posLig-1).getTl() > No.getN())  
            {
                ...
                remaneja(p,pos);
                p.setPosArq(pos, pai.getPosArq(posLig-1));
                p.setInfo(pos, pai.getInfo(posLig-1));
                p.setLig(pos, vizinho.getLig(vizinho.getTl()));   //erro!!!!!!!!!!!
                p.setTl(p.getTl()+1);
                pai.setPosArq(posLig-1, vizinho.getPosArq(vizinho.getTl()-1));
            ...
            else if(posLig < pai.getTl() && pai.getLig(posLig+1).getTl() > No.getN()) //se existir
            {
                ...
                p.setPosArq(pos, pai.getPosArq(posLig));
                p.setInfo(pos, pai.getInfo(posLig));
                p.setLig(pos+1, vizinho.getLig(0));
                p.setTl(p.getTl()+1);
                pai.setPosArq(posLig, vizinho.getPosArq(0));
            ...
            else  if(posLig > 0)//pega do lado esquerdo e faz fusão
            {
                ...
                {
                    remaneja(p, i);
                    p.setPosArq(i, vizinho.getPosArq(i));
                    p.setInfo(i, vizinho.getInfo(i));
                    p.setLig(i, vizinho.getLig(i));
                    p.setTl(p.getTl() + 1);
                }
                remaneja(p, i);
                p.setPosArq(i, pai.getPosArq(posLig-1));
            ...
            else  //pega do lado direito e faz fusão-quando não existe vizinho do lado esquerdo e do lado direito tem n elementos
            {
                ...
                {
                    remaneja(vizinho, i);
                    vizinho.setPosArq(i, p.getPosArq(i));
                    vizinho.setInfo(i, p.getInfo(i));
                    vizinho.setLig(i, p.getLig(i));
                    vizinho.setTl(vizinho.getTl() + 1);
                }
                remaneja(vizinho, i);
                vizinho.setPosArq(i, pai.getPosArq(posLig));
